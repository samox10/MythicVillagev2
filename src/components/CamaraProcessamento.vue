...
const adicionarFila = (carcaca) => {
    if (carcaca.nivelRequerido > jogo.camaraProcessamento) {
        return mostrarAviso(
            "Câmara Insuficiente", 
            `Você precisa evoluir a Câmara para o Nível ${carcaca.nivelRequerido} para processar ${carcaca.nome}.`
        );
    }
    ...
    if (slotVazio === -1) return mostrarAviso("Fila Cheia", "A fila de processamento está cheia.");
